...
# 6. Given the SNPs from the previous step, create the target list for extracting from the MSU7 reference allele file.
	...
	awk '{ped=$1; gsub(".ped","",ped); system("java -jar ~/bin/Haploview.jar -n -pedfile " $1  " -info " $2 " -skipcheck -blockoutput ALL -dprime -out " ped " -svg -png -pairwiseTagging"); print $0}' $1/$1_emmax.ps.qqman.emmax_200kb.clumped.snps_list.haploview_batch_input &> /dev/null
	printf "DONE.\n"
